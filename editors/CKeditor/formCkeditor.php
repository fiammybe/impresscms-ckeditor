...
class icmsFormCKEditor extends icms_form_elements_Textarea {
	...
	protected function initCKEditor() {
		$this->config = array(
				...
				"filebrowserImageBrowseUrl" => ICMS_URL . '/editors/CKeditor/imfinder/browse.php',
			);
	...
	public function render() {
		...
		$ret .= $xoTheme->addScript('', array('type' => 'text/javascript'),
			'var config = {filebrowserImageBrowseUrl: "' . ICMS_URL . '/editors/CKeditor/imfinder/browse.php", toolbar: "' .  $toolbar . '"}; $(function() { $("#'.@$this->_name.'_tarea").ckeditor(config); $("#'.@$this->_name.'_tarea").parents("form").submit(function() { var data = $("#'.@$this->_name.'_tarea").html(); $("#'.@$this->_name.'_tarea").html(data); }); });');
		$ret .= parent::render();
